...
//call this in the loop.  Will it cause the loop to slow down ?????
    public synchronized void getSlavebyte(){
        ...
        int uirx = (int)m_rx[0];
        if(uirx == 0){uirx = m_urixOld;}
        m_urixOld = uirx;
        if (DEBUG) { SmartDashboard.putNumber("INT recieved value", uirx); }
        
        if(uirx < 0)urixSgned = uirx + 256;//Since java doesn't have unsigned, convert if neg sign bit
        else urixSgned = uirx;
        if (DEBUG) { SmartDashboard.putNumber("sign Fixed recieved value", urixSgned); }
        /*if(urixSgned < 1 || urixSgned > 252){
            urixSgned = Float.NaN;
        }*/

        frx = (float)urixSgned;
        float rxinl = frx/2.54f;//to inches
        if (DEBUG) { SmartDashboard.putNumber("scaled recieved value", rxinl);  }   
       
        m_rxin = rxinl;
        if (DEBUG) { SmartDashboard.putNumber("recieved value", m_rxin); }
